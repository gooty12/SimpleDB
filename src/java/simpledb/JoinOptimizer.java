...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
         ...
         int joinNumber = joins.size();
         
         for (int i = 1; i <= joinNumber; i++) {
             Set<Set<LogicalJoinNode>> setOfSets = enumerateSubsets(joins, i);
             for (Set<LogicalJoinNode> singleSet : setOfSets) {
                 double currentBestPlan = Double.MAX_VALUE;
                 
                 CostCard currentBestCost = null;
                 for (LogicalJoinNode subPlan : singleSet) {
                     CostCard cost = computeCostAndCardOfSubplan(stats, filterSelectivities, subPlan, singleSet, currentBestPlan, planCache);
                     
                     if (cost != null) {
                         if (cost.cost < currentBestPlan) {
                             currentBestPlan = cost.cost;
                             currentBestCost = cost;
                         }
                         planCache.addPlan(singleSet, currentBestPlan, currentBestCost.card, currentBestCost.plan);
                     }
                 }
             }
         }

         Set<LogicalJoinNode> finalSet = this.enumerateSubsets(this.joins, joinNumber).iterator().next();

         Vector<LogicalJoinNode> orderOfJoins = planCache.bestOrders.get(finalSet);
         
         if (explain) {
             printJoins(orderOfJoins, planCache, stats, filterSelectivities);
         }
         return orderOfJoins;
    	//return joins;
    }
