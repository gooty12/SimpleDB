...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (pid == null) { // should not happen
			System.out.println("WoooooW! BTreePageId to search is null");
		}
		else {
			if (BTreePageId.LEAF == pid.pgcateg()) {
				leafPage = (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
			}
			else {
				...
				//Op op = Predicate.Op.LESS_THAN_OR_EQ;
				IndexPredicate pred = new IndexPredicate(Predicate.Op.LESS_THAN , f);
				while(iterator.hasNext()) {
					BTreeEntry entry = iterator.next();
					IndexPredicate otherPred = 
							new IndexPredicate(null, entry.getKey());
				...
				leafPage = findLeafPage(tid, dirtypages, nextPid, perm, f);
			}
		}
        return leafPage;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage newLeafPage = (BTreeLeafPage)this.getEmptyPage(tid, dirtypages, page.pid.LEAF);
		
		int totalTuples = page.getNumTuples();
				
		Iterator<Tuple> shiftRightIterator = page.reverseIterator();
		int currentTuplesInLeftLeaf = 0;
		
		//Needs to there, otherwise delete and iterator don't work together.
		Stack<Tuple> tupleStackToBePushed = new Stack<Tuple>();
		
 		while(shiftRightIterator.hasNext()) {			
			if(currentTuplesInLeftLeaf >= totalTuples/2)
				break;

			Tuple current = shiftRightIterator.next();
			tupleStackToBePushed.push(current);
			
			
			currentTuplesInLeftLeaf++;
		}
		
 		while(!tupleStackToBePushed.isEmpty()) {
 			
 			Tuple current = tupleStackToBePushed.pop();
			page.deleteTuple(current);

 			newLeafPage.insertTuple(current);
 		}
 		
		BTreePageId rightPageId = page.getRightSiblingId();
		
		if(rightPageId != null) {
			BTreeLeafPage rightSiblingPage = (BTreeLeafPage)this.getPage(tid, dirtypages, rightPageId, Permissions.READ_WRITE);
			rightSiblingPage.setLeftSiblingId(newLeafPage.pid);
		}
		page.setRightSiblingId(newLeafPage.getId());
		
		newLeafPage.setLeftSiblingId(page.getId());
		newLeafPage.setRightSiblingId(rightPageId);
			
		// why not copy the first entry in newLeafPage??
		Field fieldToCopy = newLeafPage.iterator().next().getField(this.keyField);
		
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), fieldToCopy);
		parentPage.insertEntry(new BTreeEntry(fieldToCopy, page.pid, newLeafPage.pid));
		
		this.updateParentPointer(tid, dirtypages, parentPage.pid, newLeafPage.pid);
		this.updateParentPointer(tid, dirtypages, parentPage.pid, page.pid);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int middlePoint = numberOfEntries/2;
		Iterator<BTreeEntry> reverseEntryIterator = page.reverseIterator();
		BTreeInternalPage newInternalPage = (BTreeInternalPage)this.getEmptyPage(tid, dirtypages, page.pid.INTERNAL);
		
		Stack<BTreeEntry> entriesToPush = new Stack<>();
		BTreeEntry toBePushedUp = null;
		
		BTreeEntry current = null;

		reverseEntryIterator = page.reverseIterator();
		while(reverseEntryIterator.hasNext()) {
			if(middlePoint >= numberOfEntries)
				break;

			current = reverseEntryIterator.next();
			entriesToPush.push(current);
			middlePoint++;
		}
		toBePushedUp = entriesToPush.pop();
		page.deleteKeyAndRightChild(toBePushedUp);
		//leftOfPushed = prev;
		//rightOfPushed = entryIterator.next();
		
		//leftOfPushed.setRightChild(toBePushedUp.getLeftChild());
		//rightOfPushed.setLeftChild(current.getRightChild());
		
		//newInternalPage.insertEntry(rightOfPushed);
		
		while(!entriesToPush.isEmpty()) {
			BTreeEntry currentEntry = entriesToPush.pop();
            //System.out.println("1 shifting entry " + currentEntry.toString());

			page.deleteKeyAndRightChild(currentEntry);
			
			newInternalPage.insertEntry(currentEntry);
		}
		
		
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parentPage.insertEntry(new BTreeEntry(toBePushedUp.getKey(), page.pid, newInternalPage.pid));
		
		page.setParentId(parentPage.pid);
		newInternalPage.setParentId(parentPage.pid);		
	

		this.updateParentPointers(tid, dirtypages, newInternalPage);
		this.updateParentPointers(tid, dirtypages, page);
		
		if(field.compare(Op.LESS_THAN_OR_EQ, toBePushedUp.getKey()))
			return page;
		else
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if(parentId.pgcateg() == BTreePageId.ROOT_PTR) {
			...
			// lock the parent pagesplitInternalPage
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// the corresponding parent entry.
	 	...
	 	int req = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		Deque<Tuple> tuples = new LinkedList<>();
		for (int count = 0; count < req; count++) {
			if (isRightSibling) {
				tuples.addLast(it.next());
			}
			else {
				tuples.addFirst(it.next());
			}
		}
		
		while (!tuples.isEmpty()) {
			Tuple tup = tuples.removeFirst();
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// could be redundant, but should be okay
		page = (BTreeInternalPage) this.getPage(tid, dirtypages, page.pid, Permissions.READ_WRITE);
		leftSibling = (BTreeInternalPage) getPage(tid, dirtypages, leftSibling.pid, Permissions.READ_WRITE);
		parent = (BTreeInternalPage) getPage(tid, dirtypages, parent.pid, Permissions.READ_WRITE);
		
		int req = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
		Deque<BTreeEntry> entries = new LinkedList<>();
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		for (int count = 0; count < req; count++) {
			entries.addLast(it.next());
		}
		
		// remove from parent and add it to the page first
		...
		page.insertEntry(new BTreeEntry(f, left, right));
		
		// move the req-1 entries now
		while (entries.size() > 1) {
			BTreeEntry entry = entries.remove();
			page.insertEntry(new BTreeEntry(entry.getKey(), entry.getLeftChild(), entry.getRightChild()));
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// pointers of all children in the entries that were moved.
				...
				page = (BTreeInternalPage) this.getPage(tid, dirtypages, page.pid, Permissions.READ_WRITE);
				rightSibling = (BTreeInternalPage) getPage(tid, dirtypages, rightSibling.pid, Permissions.READ_WRITE);
				parent = (BTreeInternalPage) getPage(tid, dirtypages, parent.pid, Permissions.READ_WRITE);
				
				int req = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
				...
				for (int i = 0; i < entries.size(); i++) {
					...
					BTreePageId leftChildOfEntry = entry.getLeftChild();
					
					// entry from left sibling must be moved to parent
					...
					// parent entry should be shifted to our page
					parentEntry.setRightChild(leftChildOfEntry);
					parentEntry.setLeftChild(page.reverseIterator().next().getRightChild());
					
					// insert entry from parent into page
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		leftPage = (BTreeLeafPage) getPage(tid, dirtypages, leftPage.pid, Permissions.READ_WRITE);
		rightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.pid, Permissions.READ_WRITE);
		parent = (BTreeInternalPage) getPage(tid, dirtypages, parent.pid, Permissions.READ_WRITE);
		...
		if (nextRightPageId != null) {
			BTreeLeafPage nextRightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			nextRightPage.setLeftSiblingId(leftPage.pid);
		}
		
		// rightPage.setEmptyPage()
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		leftPage = (BTreeInternalPage) getPage(tid, dirtypages, leftPage.pid, Permissions.READ_WRITE);
		rightPage = (BTreeInternalPage) getPage(tid, dirtypages, rightPage.pid, Permissions.READ_WRITE);
		parent = (BTreeInternalPage) getPage(tid, dirtypages, parent.pid, Permissions.READ_WRITE);
		...
		//pull-down
		Field f = parentEntry.getKey();
		BTreePageId left = leftPage.reverseIterator().next().getRightChild();
		BTreePageId right = rightPage.iterator().next().getLeftChild();
		leftPage.insertEntry(new BTreeEntry(f, left, right));
